...
- **Herramientas por servidor**: en Linux, scripts, test de velocidad, firewall y administración SSH segura desde una ventana con terminal interactiva. En MikroTik, el equivalente sobre RouterOS: firewall (`/ip/firewall/filter`), consola contra la REST API, scripts (`/rest/execute` o `/system/script`) y test de velocidad (`/ping` + `/tool/fetch`). Cada tipo de servidor muestra solo sus herramientas.
