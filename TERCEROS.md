...
TunnelForge se distribuye bajo **MIT**, pero interactúa con software de terceros que conserva sus propias licencias. La aplicación no relicencia esos proyectos.
...
| [golang.org/x/sys/windows](https://pkg.go.dev/golang.org/x/sys/windows) | Consulta del estado del servicio WireGuard en el SCM de Windows | BSD-3-Clause © The Go Authors |
...
Estos componentes **no se incorporan al binario de TunnelForge**. Se instalan como programas independientes en los servidores elegidos y mantienen íntegramente sus respectivas licencias:
...
En Windows, `wg-service-host.exe`, `tunnel.dll` y `wireguard.dll` se incrustan como recursos dentro del ejecutable de TunnelForge y se extraen a un directorio privado de runtime cuando se necesita iniciar un túnel. `wg-service-host.exe` es código propio MIT y únicamente carga `tunnel.dll` mediante la exportación documentada `WireGuardTunnelService`. El usuario **no instala WireGuard for Windows ni `wg.exe`**. La creación/control del servicio VPN sí requiere elevación UAC de Windows.

El workflow no compila un WireGuardNT/driver modificado: toma el `wireguard.dll` precompilado distribuido por WireGuard dentro de la dependencia oficial y conserva sus avisos y términos. TunnelForge usa únicamente las funciones públicas de la API de WireGuardNT para leer estado y estadísticas.
...
**Marca/nombre:** TunnelForge no es el cliente oficial de WireGuard. El término “WireGuard” se utiliza únicamente para identificar compatibilidad con el protocolo y los componentes correspondientes.
