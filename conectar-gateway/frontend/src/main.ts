...
var TOKEN = '';
  ...
  function renderResumen(r){
    ...
    var traffic=nodo('div','mon-kpi');traffic.appendChild(nodo('span','','Tráfico total'));var trafficRates=nodo('b','mon-kpi-net');trafficRates.appendChild(nodo('span','up','↑ '+fmtPeerRate(r.txMbit)));trafficRates.appendChild(nodo('span','down','↓ '+fmtPeerRate(r.rxMbit)));traffic.appendChild(trafficRates);traffic.appendChild(nodo('small','','Interfaz principal de los servidores'));k.appendChild(traffic);
  ...
  // ── WireGuard local ───────────────────────────────────
  var wgcProfiles:any[]=[], wgcSelected:string|null=null, wgcStatus:any={}, wgcPrev:any={}, wgcTimer:any=null, wgcSecretsVisible=false, wgcConfigAbierta=false;
  ...
  function wgcSetConfig(abrir:any){
    wgcConfigAbierta=!!abrir;
    var form=$('wgc-form'); if(form) form.hidden=!wgcConfigAbierta;
    var metrics=$('wgc-metrics'); if(metrics) metrics.hidden=wgcConfigAbierta;
    var state=$('wgc-state'); if(state) state.hidden=wgcConfigAbierta;
    ...
    if(!wgcConfigAbierta&&wgcSecretsVisible){
      ...
      var priv=$('wgc-private'); if(priv){priv.type='password';priv.value='';}
      document.querySelectorAll('#wgc-peers [data-field=presharedKey]').forEach(function(x:any){x.type='password';x.value='';});
      var rev=$('wgc-reveal'); if(rev) rev.textContent='Mostrar secretos';
    }
  ...
  function wgcPeerRow(peer?:any,index?:any){
    ...
    var head=nodo('div','wgclient-peer-head'); head.appendChild(nodo('b','',peer.name||('Peer '+((index||0)+1)))); var tools=nodo('div','wgclient-peer-head-tools');var rm=nodo('button','wgclient-peer-remove','×');rm.type='button';rm.title='Quitar peer';rm.onclick=function(){row.remove();wgcRenumberPeers();};tools.appendChild(rm);head.appendChild(tools);row.appendChild(head);
    ...
    var localWrap=nodo('div','span2 wgclient-peer-local');var localLabel=nodo('label','check');var local=document.createElement('input');local.type='checkbox';local.dataset.field='excludeLocalTraffic';local.checked=!!peer.excludeLocalTraffic;localLabel.appendChild(local);localLabel.appendChild(nodo('span','','Excluir tráfico local del túnel'));localWrap.appendChild(localLabel);localWrap.appendChild(nodo('small','','Mantiene la LAN fuera de este peer cuando usas una ruta /0. En Windows evita el kill-switch de /0 usando rutas /1 equivalentes.'));grid.appendChild(localWrap);
  ...
  function wgcCollectPeers(){var out:any[]=[];document.querySelectorAll('#wgc-peers .wgclient-peer').forEach(function(row){function v(n){var e:any=row.querySelector('[data-field='+n+']');return e?String(e.value||'').trim():'';}var local:any=row.querySelector('[data-field=excludeLocalTraffic]');out.push({name:v('name'),endpoint:v('endpoint'),publicKey:v('publicKey'),presharedKey:v('presharedKey'),allowedIPs:wgcSplit(v('allowedIPs')),persistentKeepalive:Number(v('persistentKeepalive'))||0,excludeLocalTraffic:!!(local&&local.checked)});});return out;}

  function wgcClear(){
    wgcSelected=null;wgcSecretsVisible=false;wgcSetConfig(false);$('wgc-title').textContent='Nuevo perfil';$('wgc-subtitle').textContent='Pulsa ⚙ Configuración para crear el túnel o importa un archivo .conf.';
  ...
  // ── Actualizaciones firmadas (repo público, sin token) ────────
  var updUltima = null;
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape'){
      ...
      if (document.getElementById('m-mikrotik').classList.contains('abierto')){ cerrarMikrotik(); return; }
  ...
  function pintarFwMT(r){
    var body=$('mt-fw-body'); vaciar(body);
    if(r.aviso && !r.alcanzable){ mtNota(r.aviso,'err'); }
    var reglas=(r&&r.reglas)||[];
    if(!reglas.length){ var tr=nodo('tr'); var td=nodo('td','','Sin reglas en /ip/firewall/filter.'); td.setAttribute('colspan','5'); tr.appendChild(td); body.appendChild(tr); return; }
    reglas.forEach(function(rg,i){
      var tr=nodo('tr', rg.disabled?'mt-off':'');
      tr.appendChild(nodo('td','',String(i)));
      tr.appendChild(nodo('td','',rg.chain||'—'));
      var acc=nodo('td',''); acc.appendChild(nodo('span','mt-act mt-act-'+(rg.action||'x'),rg.action||'—')); tr.appendChild(acc);
      var det=nodo('td','mt-det'); det.textContent=(rg.detalle||'')+(rg.comment?('  «'+rg.comment+'»'):''); tr.appendChild(det);
      var ops=nodo('td','mt-ops');
      if(rg.dynamic){ ops.appendChild(nodo('span','mt-dyn','dinámica')); }
      else {
        var bT=nodo('button','ghost',rg.disabled?'Activar':'Desactivar');
        bT.onclick=function(){ opFwMT({op:'toggle',id:rg.id}, bT); };
        var bD=nodo('button','peligro','Borrar');
        bD.onclick=function(){ opFwMT({op:'delete',id:rg.id}, bD); };
        ops.appendChild(bT); ops.appendChild(bD);
      }
      tr.appendChild(ops);
      body.appendChild(tr);
    });
  ...
  function cargarFwMT(){
    ...
    $('mt-fw-body').innerHTML='<tr><td colspan="5">Consultando el firewall…</td></tr>';
    api('/api/herramientas/mikrotik/firewall?servidor='+encodeURIComponent(mtServidor)).then(function(r){
      if(r.error){ mtNota(r.error,'err'); $('mt-fw-body').innerHTML='<tr><td colspan="5">'+r.error+'</td></tr>'; return; }
      pintarFwMT(r);
    }).catch(function(){ mtNota('No pude consultar el firewall.','err'); });
  }
  function opFwMT(payload, boton){
    if(!mtServidor) return;
    payload.servidor = mtServidor;
    var hacer=function(confirmar){
      if(confirmar) payload.confirmar=true;
      if(boton){ boton.disabled=true; }
      api('/api/herramientas/mikrotik/firewall',{method:'POST',body:JSON.stringify(payload)}).then(function(r){
        if(r.error){ mtNota(r.error,'err'); return; }
        if(r.necesitaConfirmar){ if(window.confirm((r.aviso||'Este cambio puede cortar el acceso.')+'\n\n¿Continuar?')){ hacer(true); } return; }
        pintarFwMT(r);
        mtNota(r.alcanzable ? 'Cambio aplicado. La API sigue respondiendo.' : (r.aviso||'Cambio aplicado, pero la API dejó de responder.'), r.alcanzable?'ok':'err');
      }).catch(function(){ mtNota('No pude aplicar el cambio.','err'); }).finally(function(){ if(boton) boton.disabled=false; });
    };
    hacer(false);
  }
  $('mt-fw-add').onclick=function(){
    if(!mtServidor) return;
    var regla={ chain:$('mt-fw-chain').value, action:$('mt-fw-action').value,
      protocol:$('mt-fw-proto').value.trim(), dstPort:$('mt-fw-dport').value.trim(),
  ...
  function abrirEditorRemoto(ruta){
    if(!fxServidor) return;
    var area=document.getElementById('fx-editor-texto');
    fxEditorRuta=''; fxEditorModificado=0; fxEditorRevision=''; area.value=''; area.disabled=true;
    ...
    api('/api/archivos',{method:'POST',body:JSON.stringify({servidor:fxServidor,accion:'leer-texto',ruta:ruta})}).then(function(r){
      if(r.error){ aviso(r.error,false); document.getElementById('fx-editor-velo').classList.remove('abierto'); return; }
      fxEditorRuta=r.ruta; fxEditorModificado=r.modificado; fxEditorRevision=r.revision||''; area.value=r.contenido||''; area.disabled=false;
      document.getElementById('fx-editor-guardar').disabled=false; area.focus();
  ...
  document.getElementById('fx-editor-guardar').onclick=function(){
    if(!fxServidor||!fxEditorRuta) return;
    ...
    api('/api/archivos',{method:'POST',body:JSON.stringify({servidor:fxServidor,accion:'guardar-texto',ruta:fxEditorRuta,contenido:document.getElementById('fx-editor-texto').value,modificado:fxEditorModificado,revision:fxEditorRevision})}).then(function(r){
      if(r.error){ aviso(r.error,false); return; }
      fxEditorModificado=r.modificado; fxEditorRevision=r.revision||''; aviso('Documento guardado en el servidor.',true); cerrarEditorRemoto(); cargarRemoto(fxRutaRem);
      ...
      .then(function(d){
        ...
        d.archivos.forEach(function(a){
          ...
          if (a.carpeta){
            ...
            var bEdi = document.createElement('button'); bEdi.textContent = 'Editar'; bEdi.title='Editar texto UTF-8 (máximo 2 MB)';
