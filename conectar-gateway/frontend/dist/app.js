...
function aviso(texto, ok) {
    ...
    clearTimeout(a._t);
    a._t = setTimeout(function () { a.style.display = 'none'; }, 4500);
}
...
function vaciar(el) { while (el.firstChild)
...
function svgAccion(tipo) {
    ...
    if (tipo === 'terminal') {
        add('rect', { x: '4', y: '5', width: '16', height: '14', rx: '1.5' });
        add('path', { d: 'M7 9l3 3-3 3' });
        add('path', { d: 'M12 15h5' });
    }
    else if (tipo === 'files') {
        add('path', { d: 'M4 7h6l2 2h8v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7z' });
        add('path', { d: 'M4 9h16' });
    }
    else if (tipo === 'tools') {
        add('path', { d: 'M14.7 6.3a4 4 0 0 0-5 5L4 17v3h3l5.7-5.7a4 4 0 0 0 5-5l-2.3 2.3-3-3 2.3-2.3z' });
    }
    else if (tipo === 'disconnect') {
        add('path', { d: 'M12 3v8' });
        add('path', { d: 'M7.2 5.8a8 8 0 1 0 9.6 0' });
    }
    return svg;
}
function botonAccion(tipo, titulo, clase) {
    var b = nodo('button', 'mini-action' + (clase ? ' ' + clase : ''), '');
    b.title = titulo;
    b.setAttribute('aria-label', titulo);
    b.appendChild(svgAccion(tipo));
    return b;
}
...
function fmtBs(b) { if (b === null || b === undefined)
    return '—'; var m = (Math.max(0, b) * 8) / 1000000; return (m < 10 ? m.toFixed(2) : m.toFixed(1)) + ' Mbit/s'; }
...
function consultarSeguridadSSH(nombre, btn, forzar) {
    ...
    if (!forzar && c && ahora - c.t < 30000) {
        rotularSeguridadSSH(btn, c.modo);
        return;
    }
...
function alternarSeguridadSSH(nombre, btn) {
    ...
    if (pass === null)
        return;
    if (!pass) {
        aviso('Necesito la contraseña actual para verificar de forma real que el login por password queda bloqueado.', false);
        return;
    }
    if (!confirm('Gateway comprobará primero la key, desactivará el login por contraseña y mantendrá root solo por key. ¿Continuar?'))
        return;
    btn.disabled = true;
    btn.textContent = 'Asegurando…';
...
function pintarConexiones(e) {
    ...
    var pv = $('pie-ver');
    if (pv)
        pv.textContent = 'v' + (e.version || '');
    var cont = $('conexiones');
    var lista = e.conexiones || [];
    var infoCon = $('info-connected');
    if (infoCon)
        infoCon.textContent = String(lista.length);
    var infoLocal = $('info-localhost');
    if (infoLocal)
        infoLocal.textContent = e.localhostSeleccionado || 'Ninguno';
    var actuales = {};
    ...
    lista.forEach(function (c) {
        ...
        if (prev) {
            ...
            if (dt > 0.5) {
                rxS = Math.max(0, (c.rx - prev.rx) / dt);
                txS = Math.max(0, (c.tx - prev.tx) / dt);
            }
        }
        ...
        var puertosActivos = {};
        (c.puertos || []).forEach(function (p) { puertosActivos[p] = true; });
...
function pintarLista(lista) {
    ...
    var infoSaved = $('info-saved');
    if (infoSaved)
        infoSaved.textContent = String((lista || []).length);
    var cta = document.getElementById('srv-cuenta');
    if (cta)
        cta.textContent = '— ' + lista.length + ' servidor' + (lista.length === 1 ? '' : 'es');
    ...
    mostrar.forEach(function (s) {
        ...
        var asa = nodo('span', 'asa', '⠿');
        asa.title = 'Arrastra para reordenar';
        div.appendChild(asa);
        div.appendChild(nodo('div', 'franja'));
        ...
        if (s.confiado)
            tags.appendChild(nodo('span', 'tag fingerprint', 'Huella verificada'));
...
if ($('btn-new-toolbar'))
    $('btn-new-toolbar').onclick = function () { abrirFormulario(); mostrarVista('profiles'); };
if ($('btn-import-toolbar'))
    $('btn-import-toolbar').onclick = function () { mostrarVista('backup'); setTimeout(function () { var el = $('imp-archivo'); if (el)
        el.click(); }, 50); };
if ($('btn-desc-todo'))
    $('btn-desc-todo').onclick = function () {
        if (!window.confirm('¿Desconectar todas las sesiones activas?'))
            return;
...
if ($('srv-view-grid'))
    $('srv-view-grid').onclick = function () { $('lista').style.gridTemplateColumns = 'repeat(2,minmax(0,1fr))'; };
if ($('srv-view-list'))
    $('srv-view-list').onclick = function () { $('lista').style.gridTemplateColumns = '1fr'; };
if ($('dash-upd-search'))
    $('dash-upd-search').onclick = function () { updBuscar(false); };
if ($('lista'))
    $('lista').style.gridTemplateColumns = 'repeat(2,minmax(0,1fr))';
...
function crearInstalarKey(aceptarHuella) {
    ...
    if (!pet.nombre || !pet.host || !pet.usuario || !pet.password) {
        aviso('Completa nombre, host, usuario y la contraseña actual del servidor.', false);
        return;
    }
    ...
    api('/api/crear-instalar-key', { method: 'POST', body: JSON.stringify(pet) }).then(function (r) {
        if (r.confirmarHuella) {
            if (confirm('Primera conexión a este servidor. Huella SSH:\n\n' + r.confirmarHuella + '\n\n¿Confiar e instalar la clave?'))
                crearInstalarKey(true);
            return;
        }
        if (r.error) {
            aviso(r.error, false);
    ...
    api('/api/conectar', { method: 'POST', body: JSON.stringify({
        ...
        if (r.ok) {
            ...
            if (r.sinTuneles) {
                ...
                if (r.puertosOmitidos)
                    msg += ' (omitidos por estar ocupados: ' + r.puertosOmitidos.join(', ') + ')';
            }
            aviso(msg, true);
            refrescarEstado();
            (r.abrirWeb || []).forEach(function (url) { abrirFuera(url); });
            if (terminalTras) {
                var srv = terminalTras;
                terminalTras = null;
...
function actualizarCuentaTuneles() { var n = document.querySelectorAll('#tun-lista .tun-fila').length; if ($('tun-cuenta'))
    $('tun-cuenta').textContent = '— ' + n + ' túnel' + (n === 1 ? '' : 'es'); }
function filaTunel(t) {
    ...
    n.type = 'text';
    n.value = t.nombre || '';
    n.placeholder = 'Nombre del servicio';
    var r = nodo('input', 'r');
    r.type = 'text';
    r.value = t.ruta || '';
    ...
    ch.type = 'checkbox';
    ch.className = 'auto-web';
    ch.checked = !!t.abrirWeb;
    ch.title = 'Abrir web al conectar';
    aw.appendChild(ch);
    var b = nodo('button', 'peligro', '✕');
    b.type = 'button';
    b.title = 'Quitar este túnel';
    b.onclick = function () { d.remove(); actualizarCuentaTuneles(); };
    d.appendChild(p);
    d.appendChild(n);
    d.appendChild(r);
    d.appendChild(aw);
...
function leerTunelesForm() { var out = []; document.querySelectorAll('#tun-lista .tun-fila').forEach(function (f) { var p = parseInt(f.querySelector('.p').value); if (!p)
    return; out.push({ puerto: p, nombre: f.querySelector('.n').value.trim(), ruta: f.querySelector('.r').value.trim(), abrirWeb: f.querySelector('.auto-web').checked }); }); return out; }
...
function abrirTerminal(nombre, comandoInicial, comandoSilencioso) {
        ...
        if (comandoInicial) {
            if (comandoSilencioso) {
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        if (!term || !wsTerm || wsTerm.readyState !== 1)
                            return;
                        term.clear();
    ...
    term.onResize(function (t) {
        if (wsTerm && wsTerm.readyState === 1)
            wsTerm.send(JSON.stringify({ cols: t.cols, rows: t.rows }));
    });
    window.addEventListener('resize', ajustarTerm);
}
function ajustarTerm() { if (fit)
    fit.fit(); }
...
function pintarHistorialTerminal(lista) {
    ...
    if (!lista || !lista.length) {
        mensajeEn(cont, 'vacio-hist', 'Sin comandos todavía.');
        return;
    }
    vaciar(cont);
...
function guardarComandoHistorial(nombre, comando) {
    api('/api/historial', { method: 'POST', body: JSON.stringify({ nombre: nombre, comando: comando }) })
        .then(function () { if (servidorTerminal === nombre)
        cargarHistorialTerminal(nombre); });
}
// ── Copia de seguridad ────────────────────────────────────────
...
document.getElementById('btn-exportar').onclick = function () {
    ...
    if (pass.length < 8) {
        aviso('La contraseña debe tener al menos 8 caracteres.', false);
        return;
    }
    var b = this;
    b.disabled = true;
    ...
    api('/api/exportar', { method: 'POST', body: JSON.stringify({
        ...
        if (r.cancelado) {
            res.textContent = 'Exportación cancelada.';
            return;
        }
...
document.getElementById('btn-importar').onclick = function () {
    ...
    if (!archivo) {
        aviso('Elige el archivo .cgw a importar.', false);
        return;
    }
    if (!pass) {
        aviso('Escribe la contraseña de la copia.', false);
        return;
    }
    var b = this;
    b.disabled = true;
...
function monNota(txt, tipo) { var e = $('mon-note'); if (!e)
    return; e.textContent = txt || ''; e.className = 'monitor-note' + (tipo ? ' ' + tipo : ''); }
function renderMonitoring(data) {
    monCache = data;
    var cfg = data.config || {};
    var servers = data.servidores || [];
    var sel = $('mon-server');
    var previo = sel.value;
    vaciar(sel);
    var vac = nodo('option', '', 'Selecciona servidor…');
    vac.value = '';
    sel.appendChild(vac);
    servers.forEach(function (s) { var o = nodo('option', '', s.nombre + ' · ' + s.host); o.value = s.nombre; sel.appendChild(o); });
    sel.value = cfg.monitorServer || previo || '';
    $('mon-port-start').value = cfg.portStart || 19100;
    $('mon-port-end').value = cfg.portEnd || 19999;
    var list = $('mon-server-list');
    vaciar(list);
    var targets = cfg.targets || [];
    var targetNames = {};
    targets.forEach(function (t) { targetNames[t.servidor] = t; });
    servers.forEach(function (s) {
        var row = nodo('label', 'mon-server-row');
        var cb = document.createElement('input');
        cb.type = 'checkbox';
        cb.dataset.server = s.nombre;
        cb.checked = !!targetNames[s.nombre];
        row.appendChild(cb);
        var main = nodo('span', 'mon-server-main');
        main.appendChild(nodo('b', '', s.nombre));
        main.appendChild(nodo('small', '', s.host + ':' + s.puerto + ' · ' + s.usuario));
        row.appendChild(main);
        var state = nodo('span', 'mon-server-state' + (s.monitorizado ? ' on' : ''), s.monitorizado ? ('● ' + (targetNames[s.nombre].localPort || 9100)) : (s.conectado ? 'Conectado' : 'Conectar primero'));
        row.appendChild(state);
        if (s.monitorizado) {
            var d = nodo('button', 'mon-diag-btn', 'Diagnosticar');
            d.type = 'button';
            d.dataset.server = s.nombre;
            d.onclick = function (ev) { ev.preventDefault(); ev.stopPropagation(); var btn = this; btn.disabled = true; monNota('Diagnosticando ' + btn.dataset.server + '…', ''); api('/api/monitoring/diagnostico', { method: 'POST', body: JSON.stringify({ servidor: btn.dataset.server }) }).then(function (r) { if (r.error) {
                monNota(r.error, 'err');
                return;
            } var txt = (r.pasos || []).map(function (x) { return (x.ok ? '✓ ' : '✗ ') + x.nombre; }).join(' · '); monNota(btn.dataset.server + ': ' + txt, r.saludable ? 'ok' : 'err'); }).finally(function () { btn.disabled = false; }); };
            row.appendChild(d);
        }
    ...
    var map = $('mon-port-map');
    vaciar(map);
    if (!targets.length)
        map.appendChild(nodo('div', '', 'Sin túneles de monitoreo todavía.'));
    targets.forEach(function (t) { map.appendChild(nodo('div', '', t.servidor + ' · 127.0.0.1:' + t.localPort + ' → SSH → :' + t.remotePort)); });
    if (monDashboard === 'overview')
        cargarResumen();
    else if (monDashboard === 'wg')
        cargarPeers();
}
function cargarMonitoring() { return api('/api/monitoring/estado').then(function (r) { if (r.error) {
    monNota(r.error, 'err');
    return;
} renderMonitoring(r); }).catch(function () { monNota('No pude cargar la configuración de monitoreo.', 'err'); }); }
function monRenderProgress(p) {
    var box = $('mon-progress');
    if (!box)
        return;
    if (!p || (!p.activo && !p.mostrar)) {
        box.hidden = true;
        return;
    }
    box.hidden = false;
    $('mon-progress-title').textContent = p.operacion || 'Monitoreo';
    $('mon-progress-pct').textContent = (p.porcentaje || 0) + '%';
    $('mon-progress-bar').style.width = (p.porcentaje || 0) + '%';
    $('mon-progress-stage').textContent = p.etapa || '';
    var log = $('mon-progress-log');
    log.textContent = (p.log || []).join('\n');
    log.scrollTop = log.scrollHeight;
}
function monPollProgress() {
    if (monProgressTimer)
        clearInterval(monProgressTimer);
    var tick = function () { api('/api/monitoring/progreso').then(function (r) { if (!r.error)
        monRenderProgress(r); }); };
    tick();
    monProgressTimer = setInterval(tick, 700);
}
function monStopProgress() { if (monProgressTimer) {
    clearInterval(monProgressTimer);
    monProgressTimer = null;
} api('/api/monitoring/progreso').then(function (r) { if (!r.error)
    monRenderProgress(r); }); }
function fmtPct(v) { v = Number(v) || 0; return v.toFixed(v < 10 ? 1 : 0) + '%'; }
function fmtUptime(s) { s = Number(s) || 0; if (s <= 0)
    return '—'; var d = Math.floor(s / 86400), h = Math.floor((s % 86400) / 3600); return d ? d + 'd ' + h + 'h' : h + 'h'; }
function renderResumen(r) {
    ...
    if (!arr.length) {
        list.appendChild(nodo('div', 'mon-peer-empty', 'Selecciona servidores y aplica el monitoreo para ver métricas.'));
        return;
    }
    arr.forEach(function (x) {
        var card = nodo('div', 'mon-health-card');
        var name = nodo('div', 'mon-health-name');
        var title = nodo('b', '');
        var dot = nodo('i', 'mon-live-dot' + (x.online ? ' ok' : ''));
        title.appendChild(dot);
        title.appendChild(document.createTextNode(x.nombre || 'Servidor'));
        name.appendChild(title);
        name.appendChild(nodo('small', '', x.online ? ('Uptime ' + fmtUptime(x.uptime)) : 'Sin métricas'));
...
function cargarResumen() { if (monDashboard !== 'overview')
    return; api('/api/monitoring/resumen').then(function (r) { if (r.error) {
    var l = $('mon-health-list');
    vaciar(l);
    l.appendChild(nodo('div', 'mon-peer-empty', r.error));
    return;
} renderResumen(r); }); }
function fmtPeerRate(v) { v = Number(v) || 0; return (v < 10 ? v.toFixed(2) : v.toFixed(1)) + ' Mbit/s'; }
function fmtHandshake(age) { age = Number(age); if (!isFinite(age) || age < 0)
    return '—'; if (age < 60)
    return Math.round(age) + 's'; if (age < 3600)
    return Math.floor(age / 60) + 'm'; if (age < 86400)
    return Math.floor(age / 3600) + 'h'; return Math.floor(age / 86400) + 'd'; }
function renderPeers(data) {
    var list = $('mon-peer-list');
    vaciar(list);
    var peers = ((data && data.peers) || []).slice();
    var q = (($('mon-peer-search') && $('mon-peer-search').value) || '').trim().toLowerCase();
    if (q)
        peers = peers.filter(function (p) { return ((p.nombre || '') + ' ' + (p.servidor || '')).toLowerCase().indexOf(q) >= 0; });
    var ord = ($('mon-peer-sort') && $('mon-peer-sort').value) || 'name';
    peers.sort(function (a, b) { if (ord === 'rx')
        return (Number(b.rxMbit) || 0) - (Number(a.rxMbit) || 0); if (ord === 'tx')
        return (Number(b.txMbit) || 0) - (Number(a.txMbit) || 0); if (ord === 'active')
        return (Number(a.handshakeAge) || 1e15) - (Number(b.handshakeAge) || 1e15); return (a.nombre || '').localeCompare(b.nombre || ''); });
    if (!peers.length) {
        list.appendChild(nodo('div', 'mon-peer-empty', 'No hay peers WireGuard con métricas todavía.'));
        return;
    }
...
function cargarPeers() { if (monDashboard !== 'wg')
    return; api('/api/monitoring/peers').then(function (r) { if (r.error) {
    var l = $('mon-peer-list');
    vaciar(l);
    l.appendChild(nodo('div', 'mon-peer-empty', r.error));
    return;
...
function monSetTab(tab) {
    monDashboard = tab;
    var wg = tab === 'wg', summary = tab === 'overview';
    $('mon-tab-wg').classList.toggle('activo', wg);
    $('mon-tab-overview').classList.toggle('activo', summary);
    $('mon-peer-view').hidden = !wg;
    $('mon-summary-view').hidden = !summary;
    $('mon-wg-legend').hidden = !wg;
    $('mon-view-label').textContent = wg ? 'Peers en tiempo real' : 'Vista rápida nativa';
    if (monPeerTimer) {
        clearInterval(monPeerTimer);
        monPeerTimer = null;
    }
    if (monSummaryTimer) {
        clearInterval(monSummaryTimer);
        monSummaryTimer = null;
    }
    if (wg) {
        cargarPeers();
        monPeerTimer = setInterval(cargarPeers, 3000);
    }
    else {
        cargarResumen();
        monSummaryTimer = setInterval(cargarResumen, 5000);
    }
}
$('mon-refresh').onclick = cargarMonitoring;
$('mon-save').onclick = function () { var b = this; b.disabled = true; monNota('Guardando configuración…', ''); api('/api/monitoring/config', { method: 'POST', body: JSON.stringify({ monitorServer: $('mon-server').value, portStart: Number($('mon-port-start').value), portEnd: Number($('mon-port-end').value) }) }).then(function (r) { if (r.error) {
    monNota(r.error, 'err');
    return;
} monNota('Configuración cifrada y guardada.', 'ok'); return cargarMonitoring(); }).finally(function () { b.disabled = false; }); };
$('mon-prepare').onclick = function () { var b = this; b.disabled = true; monNota('Preparando servidor de monitoreo…', ''); monPollProgress(); api('/api/monitoring/preparar', { method: 'POST', body: '{}' }).then(function (r) { if (r.error) {
    monNota(r.error, 'err');
    return;
} monNota(r.mensaje || 'Servidor monitor preparado.', 'ok'); return cargarMonitoring(); }).finally(function () { b.disabled = false; monStopProgress(); }); };
$('mon-apply').onclick = function () { var elegidos = []; document.querySelectorAll('#mon-server-list input[type=checkbox]:checked').forEach(function (c) { elegidos.push(c.dataset.server); }); var b = this; b.disabled = true; monNota('Aplicando selección…', ''); monPollProgress(); api('/api/monitoring/targets', { method: 'POST', body: JSON.stringify({ servidores: elegidos }) }).then(function (r) { if (r.error) {
    monNota(r.error, 'err');
    return;
} monNota('Monitoreo aplicado. Prometheus ya tiene ' + (r.targets || []).length + ' target(s).', 'ok'); return cargarMonitoring(); }).finally(function () { b.disabled = false; monStopProgress(); }); };
$('mon-peer-search').oninput = cargarPeers;
$('mon-peer-sort').onchange = cargarPeers;
$('mon-tab-overview').onclick = function () { monSetTab('overview'); };
$('mon-tab-wg').onclick = function () { monSetTab('wg'); };
$('mon-reload').onclick = function () { if (monDashboard === 'wg')
    cargarPeers();
else
    cargarResumen(); };
$('mon-peer-refresh').onclick = cargarPeers;
// ── WireGuard local ───────────────────────────────────
var wgcProfiles = [], wgcSelected = null, wgcStatus = {}, wgcPrev = {}, wgcTimer = null, wgcSecretsVisible = false, wgcConfigAbierta = false;
// La vista de WireGuard muestra por defecto SOLO estado y estadísticas del
// túnel. El formulario completo (keys, peers, hooks) aparece únicamente
// cuando el usuario pulsa "⚙ Configuración".
function wgcSetConfig(abrir) {
    wgcConfigAbierta = !!abrir;
    var form = $('wgc-form');
    if (form)
    ...
    if (btn) {
        ...
        btn.className = 'wgclient-config-btn' + (wgcConfigAbierta ? ' abierto' : '');
        btn.setAttribute('aria-expanded', wgcConfigAbierta ? 'true' : 'false');
        btn.title = wgcConfigAbierta ? 'Volver al estado del túnel' : 'Abrir la configuración del perfil';
    }
    if (!wgcConfigAbierta && wgcSecretsVisible) {
        // Al cerrar la configuración no dejamos secretos revelados en el DOM.
        wgcSecretsVisible = false;
        var priv = $('wgc-private');
        if (priv) {
            priv.type = 'password';
            priv.value = '';
        }
        document.querySelectorAll('#wgc-peers [data-field=presharedKey]').forEach(function (x) { x.type = 'password'; x.value = ''; });
        var rev = $('wgc-reveal');
        if (rev)
...
function wgcSplit(v) { return String(v || '').split(/[,\n]/).map(function (x) { return x.trim(); }).filter(Boolean); }
function wgcLines(v) { return String(v || '').split(/\r?\n/).map(function (x) { return x.trim(); }).filter(Boolean); }
function wgcBytes(v) { v = Math.max(0, Number(v) || 0); var u = ['B', 'KB', 'MB', 'GB', 'TB'], i = 0; while (v >= 1024 && i < u.length - 1) {
    v /= 1024;
    i++;
} return (v >= 100 ? Math.round(v) : v.toFixed(1)) + ' ' + u[i]; }
function wgcHandshake(ts) { var n = Number(ts) || 0; if (!n)
    return '—'; var d = Math.max(0, Math.floor(Date.now() / 1000 - n)); if (d < 60)
    return d + ' s'; if (d < 3600)
    return Math.floor(d / 60) + ' min'; if (d < 86400)
    return Math.floor(d / 3600) + ' h'; return Math.floor(d / 86400) + ' d'; }
function wgcMbit(bytesPerSec) { var x = Math.max(0, Number(bytesPerSec) || 0) * 8 / 1000000; return (x < 10 ? x.toFixed(2) : x.toFixed(1)) + ' Mbit/s'; }
function wgcProfileById(id) { return wgcProfiles.find(function (p) { return p.id === id; }) || null; }
function wgcEndpoint(p) { var peers = (p && p.peers) || []; for (var i = 0; i < peers.length; i++) {
    if (peers[i].endpoint)
        return peers[i].endpoint;
} return 'sin endpoint'; }
function wgcPeerRow(peer, index) {
    peer = peer || {};
    var row = nodo('div', 'wgclient-peer');
    row.dataset.index = String(index || 0);
    ...
    rm.onclick = function () { row.remove(); wgcRenumberPeers(); };
    tools.appendChild(rm);
    head.appendChild(tools);
    row.appendChild(head);
    var grid = nodo('div', 'wgclient-peer-grid');
    function field(label, id, val, ph, span, type) { var d = nodo('div', span ? 'span2' : ''); var l = nodo('label', '', label); d.appendChild(l); var inp = document.createElement('input'); inp.type = type || 'text'; inp.dataset.field = id; inp.value = val || ''; if (ph)
        inp.placeholder = ph; d.appendChild(inp); grid.appendChild(d); return inp; }
    var name = field('Nombre amigable', 'name', peer.name || '', 'Casa / Oficina / Gateway');
    name.oninput = function () { head.querySelector('b').textContent = name.value || ('Peer ' + (Number(row.dataset.index) + 1)); };
    ...
    var pskWrap = nodo('div', 'span2');
    pskWrap.appendChild(nodo('label', '', 'PresharedKey (opcional)'));
    var pskRow = nodo('div', 'wgclient-copy-row');
    var psk = document.createElement('input');
    psk.type = 'password';
    psk.dataset.field = 'presharedKey';
    psk.placeholder = peer.hasPresharedKey ? 'Guardada cifrada · vacío = conservar' : 'opcional';
    psk.autocomplete = 'off';
    pskRow.appendChild(psk);
    var genPSK = nodo('button', '', 'Generar PSK');
    genPSK.type = 'button';
    genPSK.onclick = function () { genPSK.disabled = true; api('/api/wireguard/generar-psk', { method: 'POST', body: '{}' }).then(function (r) { if (r.error) {
        wgcNote(r.error, 'err');
        return;
    } psk.value = r.presharedKey || ''; psk.type = 'text'; wgcNote('PresharedKey generada localmente. Guarda el perfil para cifrarla.', 'warn'); }).finally(function () { genPSK.disabled = false; }); };
    pskRow.appendChild(genPSK);
    pskWrap.appendChild(pskRow);
    grid.appendChild(pskWrap);
    ...
    var localWrap = nodo('div', 'span2 wgclient-peer-local');
    var localLabel = nodo('label', 'check');
    var local = document.createElement('input');
    local.type = 'checkbox';
    local.dataset.field = 'excludeLocalTraffic';
    local.checked = !!peer.excludeLocalTraffic;
    localLabel.appendChild(local);
    localLabel.appendChild(nodo('span', '', 'Excluir tráfico local del túnel'));
    localWrap.appendChild(localLabel);
    localWrap.appendChild(nodo('small', '', 'Mantiene la LAN fuera de este peer cuando usas una ruta /0. En Windows evita el kill-switch de /0 usando rutas /1 equivalentes.'));
    grid.appendChild(localWrap);
    row.appendChild(grid);
    return row;
}
function wgcRenumberPeers() { document.querySelectorAll('#wgc-peers .wgclient-peer').forEach(function (row, i) { row.dataset.index = String(i); var b = row.querySelector('.wgclient-peer-head b'); var n = row.querySelector('[data-field=name]'); if (b && !n.value)
    b.textContent = 'Peer ' + (i + 1); }); }
function wgcRenderPeers(peers) { var box = $('wgc-peers'); vaciar(box); (peers || []).forEach(function (p, i) { box.appendChild(wgcPeerRow(p, i)); }); if (!box.children.length)
    box.appendChild(wgcPeerRow({}, 0)); }
function wgcCollectPeers() { var out = []; document.querySelectorAll('#wgc-peers .wgclient-peer').forEach(function (row) { function v(n) { var e = row.querySelector('[data-field=' + n + ']'); return e ? String(e.value || '').trim() : ''; } var local = row.querySelector('[data-field=excludeLocalTraffic]'); out.push({ name: v('name'), endpoint: v('endpoint'), publicKey: v('publicKey'), presharedKey: v('presharedKey'), allowedIPs: wgcSplit(v('allowedIPs')), persistentKeepalive: Number(v('persistentKeepalive')) || 0, excludeLocalTraffic: !!(local && local.checked) }); }); return out; }
function wgcClear() {
    wgcSelected = null;
    wgcSecretsVisible = false;
    wgcSetConfig(false);
    ...
    $('wgc-hooks-allow').checked = false;
    wgcRenderPeers([]);
    $('wgc-export').hidden = true;
    $('wgc-delete').hidden = true;
    wgcSetSnapshot({ connected: false, interface: '—', rxBytes: 0, txBytes: 0 });
    wgcNote('Las claves privadas y PresharedKey se guardan cifradas. El archivo .conf exportado sí contiene secretos.', '');
    wgcRenderList();
}
function wgcLoadProfile(id) { var p = wgcProfileById(id); if (!p)
    return; var cambioDePerfil = wgcSelected !== id; wgcSelected = id; wgcSecretsVisible = false; if (cambioDePerfil)
    wgcSetConfig(false); $('wgc-title').textContent = p.name; $('wgc-subtitle').textContent = (p.addresses || []).join(', ') + ' · ' + wgcEndpoint(p); $('wgc-name').value = p.name || ''; $('wgc-address').value = (p.addresses || []).join(', '); $('wgc-private').value = ''; $('wgc-private').type = 'password'; $('wgc-public').value = p.publicKey || ''; $('wgc-dns').value = (p.dns || []).join(', '); $('wgc-mtu').value = p.mtu || ''; $('wgc-listen').value = p.listenPort || ''; $('wgc-table').value = p.table || ''; $('wgc-notes').value = p.notes || ''; $('wgc-preup').value = (p.preUp || []).join('\n'); $('wgc-postup').value = (p.postUp || []).join('\n'); $('wgc-predown').value = (p.preDown || []).join('\n'); $('wgc-postdown').value = (p.postDown || []).join('\n'); $('wgc-auto').checked = !!p.autoConnect; $('wgc-hooks-allow').checked = !!p.allowHooks; wgcRenderPeers(p.peers || []); $('wgc-export').hidden = false; $('wgc-delete').hidden = false; wgcRenderList(); var st = wgcStatus[id] && wgcStatus[id].snapshot; wgcSetSnapshot(st || { connected: false, interface: p.interface }); }
function wgcRenderList() { var box = $('wgc-profile-list'); vaciar(box); var q = String($('wgc-search').value || '').toLowerCase(); var arr = wgcProfiles.filter(function (p) { return !q || String(p.name).toLowerCase().includes(q) || wgcEndpoint(p).toLowerCase().includes(q); }); if (!arr.length) {
...
function wgcApplyStatus(data) { if (!data)
    ...
    if (dt > .3) {
        rx = Math.max(0, (Number(snap.rxBytes || 0) - prev.rx) / dt);
        tx = Math.max(0, (Number(snap.txBytes || 0) - prev.tx) / dt);
        (snap.peers || []).forEach(function (pp) { var old = prev.peers && prev.peers[pp.publicKey]; if (old) {
            peerRates[pp.publicKey] = { rx: Math.max(0, (Number(pp.rxBytes || 0) - old.rx) / dt), tx: Math.max(0, (Number(pp.txBytes || 0) - old.tx) / dt) };
        } });
    }
} var peerPrev = {}; (snap.peers || []).forEach(function (pp) { peerPrev[pp.publicKey] = { rx: Number(pp.rxBytes || 0), tx: Number(pp.txBytes || 0) }; }); wgcPrev[x.id] = { t: now, rx: Number(snap.rxBytes || 0), tx: Number(snap.txBytes || 0), peers: peerPrev }; x._rate = { rx: rx, tx: tx }; x._peerRates = peerRates; wgcStatus[x.id] = x; }); wgcRenderList(); if (wgcSelected) {
    var st = wgcStatus[wgcSelected];
    wgcSetSnapshot(st ? st.snapshot : { connected: false, interface: (wgcProfileById(wgcSelected) || {}).interface });
} }
function wgcPoll() { api('/api/wireguard/estado').then(wgcApplyStatus).catch(function () { }); }
function cargarWireGuard() { api('/api/wireguard/perfiles').then(function (p) { wgcProfiles = p || []; if (wgcSelected && !wgcProfileById(wgcSelected))
    wgcSelected = null; if (!wgcSelected && wgcProfiles.length)
    wgcSelected = wgcProfiles[0].id; if (wgcSelected)
    wgcLoadProfile(wgcSelected);
else
    wgcClear(); wgcPoll(); if (wgcTimer)
    clearInterval(wgcTimer); wgcTimer = setInterval(function () { if (vistaActual === 'wireguard')
    wgcPoll(); }, 2000); }).catch(function () { wgcNote('No pude cargar los perfiles WireGuard.', 'err'); }); }
function wgcSave(connectAfter) { var req = { id: wgcSelected || '', name: $('wgc-name').value.trim(), privateKey: $('wgc-private').value.trim(), addresses: wgcSplit($('wgc-address').value), dns: wgcSplit($('wgc-dns').value), mtu: Number($('wgc-mtu').value) || 0, listenPort: Number($('wgc-listen').value) || 0, table: $('wgc-table').value.trim(), notes: $('wgc-notes').value, autoConnect: !!$('wgc-auto').checked, allowHooks: !!$('wgc-hooks-allow').checked, preUp: wgcLines($('wgc-preup').value), postUp: wgcLines($('wgc-postup').value), preDown: wgcLines($('wgc-predown').value), postDown: wgcLines($('wgc-postdown').value), peers: wgcCollectPeers() }; var b = $('wgc-save'); b.disabled = true; wgcNote('Guardando perfil cifrado…', ''); return api('/api/wireguard/perfiles', { method: 'POST', body: JSON.stringify(req) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    throw new Error(r.error);
} wgcSelected = r.profile.id; $('wgc-private').value = ''; wgcNote('Perfil guardado. PrivateKey y PresharedKey quedaron cifradas.', 'ok'); return api('/api/wireguard/perfiles'); }).then(function (list) { wgcProfiles = list || []; wgcLoadProfile(wgcSelected); wgcSetConfig(false); if (connectAfter)
    return wgcConnectNow(); return; }).finally(function () { b.disabled = false; }); }
function wgcConnectNow() { if (!wgcSelected)
    return Promise.resolve(); var b = $('wgc-connect'); b.disabled = true; b.textContent = 'Conectando…'; wgcNote('Activando túnel WireGuard integrado…', ''); return api('/api/wireguard/conectar', { method: 'POST', body: JSON.stringify({ id: wgcSelected }) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
...
$('wgc-generate').onclick = function () { api('/api/wireguard/generar-key', { method: 'POST', body: '{}' }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} $('wgc-private').value = r.privateKey; $('wgc-private').type = 'text'; $('wgc-public').value = r.publicKey; wgcSecretsVisible = true; wgcNote('Nuevo par de keys generado localmente. Guarda el perfil para cifrar la privada.', 'warn'); }); };
$('wgc-reveal').onclick = function () { if (!wgcSelected) {
    $('wgc-private').type = $('wgc-private').type === 'password' ? 'text' : 'password';
    return;
} if (wgcSecretsVisible) {
    wgcSecretsVisible = false;
    $('wgc-private').type = 'password';
    $('wgc-private').value = '';
    document.querySelectorAll('#wgc-peers [data-field=presharedKey]').forEach(function (x) { x.type = 'password'; x.value = ''; });
    this.textContent = 'Mostrar secretos';
    return;
} if (!confirm('La clave privada se mostrará en pantalla. ¿Continuar?'))
    return; api('/api/wireguard/revelar', { method: 'POST', body: JSON.stringify({ id: wgcSelected }) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} wgcSecretsVisible = true; $('wgc-private').type = 'text'; $('wgc-private').value = r.privateKey || ''; document.querySelectorAll('#wgc-peers [data-field=presharedKey]').forEach(function (x, i) { x.type = 'text'; x.value = (r.presharedKeys || [])[i] || ''; }); $('wgc-reveal').textContent = 'Ocultar secretos'; }); };
$('wgc-copy-public').onclick = function () { var v = $('wgc-public').value; if (!v)
    return; if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(v);
    wgcNote('Clave pública copiada.', 'ok');
}
else {
    var e = $('wgc-public');
    e.select();
...
$('wgc-save').onclick = function () { wgcSave(false).catch(function () { }); };
// Con la configuración cerrada no hay nada visible que guardar: se conecta directo.
$('wgc-connect').onclick = function () { if (wgcConfigAbierta) {
    wgcSave(true).catch(function () { });
}
else {
    wgcConnectNow();
} };
...
$('wgc-disconnect').onclick = function () { if (!wgcSelected)
    ...
    wgcNote(r.error, 'err');
    return;
} wgcNote('Túnel desconectado.', 'ok'); wgcPoll(); }).finally(function () { b.disabled = false; b.textContent = 'Desconectar'; }); };
$('wgc-full-tunnel').onclick = function () { var rows = document.querySelectorAll('#wgc-peers .wgclient-peer'); if (!rows.length) {
    $('wgc-add-peer').click();
    rows = document.querySelectorAll('#wgc-peers .wgclient-peer');
} var ip = rows[0].querySelector('[data-field=allowedIPs]'); ip.value = '0.0.0.0/0, ::/0'; wgcNote('El primer peer quedó como túnel completo IPv4 + IPv6. Activa “Excluir tráfico local” en ese peer si quieres conservar acceso a la LAN.', 'warn'); };
$('wgc-import').onclick = function () { $('wgc-import-file').click(); };
$('wgc-import-file').onchange = function () { var f = this.files && this.files[0]; if (!f)
    return; var rd = new FileReader(); rd.onload = function () { var nm = f.name.replace(/\.conf$/i, ''); api('/api/wireguard/importar', { method: 'POST', body: JSON.stringify({ name: nm, content: String(rd.result || '') }) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} wgcSelected = r.profile.id; var warns = r.warnings || []; wgcNote('Perfil importado' + (warns.length ? ' · ' + warns.join(' · ') : '.'), 'warn'); cargarWireGuard(); }); }; rd.readAsText(f); this.value = ''; };
$('wgc-export').onclick = function () { if (!wgcSelected)
    return; if (!confirm('El archivo .conf exportado contiene la PrivateKey en texto legible. ¿Exportar?'))
    return; api('/api/wireguard/exportar', { method: 'POST', body: JSON.stringify({ id: wgcSelected }) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} if (r.cancelado) {
    wgcNote('Exportación cancelada.', '');
    return;
} wgcNote('Perfil exportado en ' + r.path, 'ok'); }); };
$('wgc-delete').onclick = function () { if (!wgcSelected)
    return; var p = wgcProfileById(wgcSelected); if (!confirm('¿Eliminar el perfil WireGuard "' + (p ? p.name : '') + '"?'))
    return; api('/api/wireguard/eliminar', { method: 'POST', body: JSON.stringify({ id: wgcSelected }) }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} wgcSelected = null; cargarWireGuard(); wgcNote('Perfil eliminado.', 'ok'); }); };
$('wgc-engine-install').onclick = function () { var b = this; b.disabled = true; b.textContent = 'Preparando…'; api('/api/wireguard/motor/instalar', { method: 'POST', body: '{}' }).then(function (r) { if (r.error) {
    wgcNote(r.error, 'err');
    return;
} wgcNote(r.message || 'Motor preparado. Vuelve a comprobar después de instalar.', 'warn'); setTimeout(wgcPoll, 1500); }).finally(function () { b.disabled = false; b.textContent = 'Instalar herramientas WireGuard (Linux)'; }); };
...
function updSemaforo(estado, resumen) {
    ...
    p.title = resumen;
    var p2 = $('upd-punto-dash');
    if (p2) {
        p2.className = 'upd-punto ' + estado;
        p2.title = resumen;
    }
    var s2 = $('upd-dash-status');
    if (s2)
        s2.textContent = resumen || 'sin comprobar';
    var is = $('info-update-status');
    if (is)
        is.textContent = resumen || 'Actualizaciones firmadas';
    var next = $('dash-next-check');
    if (next) {
        next.textContent = estado === 'rojo' ? 'Hay una nueva versión disponible.' : (estado === 'verde' ? 'Tu instalación está verificada y al día.' : 'Pendiente de comprobar o falta token.');
    }
}
...
function updBuscar(silencioso) {
    if (!silencioso)
        updEstado('Buscando la última Release privada…', '');
    updSemaforo('amarillo', 'comprobando…');
    return api('/api/actualizaciones', { method: 'POST', body: JSON.stringify({ accion: 'buscar' }) }).then(function (r) {
        if (r.error) {
            if (!silencioso)
                updEstado(r.error, 'err');
            updSemaforo('amarillo', 'no se pudo comprobar');
            return;
        }
        updUltima = r;
        $('upd-instalar').style.display = r.disponible ? 'inline-block' : 'none';
        $('upd-notas').textContent = r.notas || '';
        $('upd-notas').style.display = r.notas ? 'block' : 'none';
        if (r.disponible) {
            updEstado('Nueva versión v' + r.nueva + ' disponible. Firma verificada.', 'err');
            updSemaforo('rojo', 'v' + r.nueva + ' disponible');
        }
        else {
            updEstado('Estás al día: v' + r.actual + '. Firma de la Release verificada.', 'ok');
            updSemaforo('verde', 'estás al día');
        }
    }).catch(function () { if (!silencioso)
        updEstado('No pude consultar GitHub.', 'err'); updSemaforo('amarillo', 'sin comprobar'); });
}
api('/api/version').then(function (v) {
    $('app-ver').textContent = 'v' + v.version;
    var dv = $('dash-app-ver');
    if (dv)
        dv.textContent = 'v' + v.version;
    var iv = $('info-version');
    if (iv)
        iv.textContent = 'v' + v.version;
...
api('/api/actualizaciones').then(function (c) {
    ...
    if (c.tokenConfigurado && c.buscarAlInicio)
        updBuscar(true);
}).catch(function () { });
$('upd-guardar').onclick = function () {
    var token = $('upd-token').value.trim();
    if (!token) {
        updEstado('Pega primero el token de GitHub.', 'err');
        return;
    }
    var b = this;
    b.disabled = true;
    updEstado('Guardando y comprobando acceso al repo privado…', '');
    api('/api/actualizaciones', { method: 'POST', body: JSON.stringify({ accion: 'guardar-token', token: token }) }).then(function (r) {
        $('upd-token').value = '';
        if (r.error) {
            updEstado(r.error, 'err');
            updSemaforo('amarillo', 'token sin verificar');
            return;
        }
        updEstado('Token guardado y acceso verificado.', 'ok');
...
$('upd-borrar').onclick = function () { api('/api/actualizaciones', { method: 'POST', body: JSON.stringify({ accion: 'borrar-token' }) }).then(function (r) { if (r.error) {
    updEstado(r.error, 'err');
    return;
} $('upd-instalar').style.display = 'none'; $('upd-notas').style.display = 'none'; updEstado('Token borrado.', 'ok'); updSemaforo('amarillo', 'token no configurado'); }); };
...
$('upd-instalar').onclick = function () {
    if (!updUltima || !updUltima.disponible)
        return;
    if (!window.confirm('Instalar Gateway WISP Access v' + updUltima.nueva + ' ahora? La app se cerrará y volverá a abrir sola.'))
        return;
    var b = this;
    b.disabled = true;
    updEstado('Descargando, verificando firma y SHA-256…', '');
    api('/api/actualizaciones', { method: 'POST', body: JSON.stringify({ accion: 'instalar' }) }).then(function (r) {
        ...
        updEstado('Actualización verificada. Reiniciando en v' + r.version + '…', 'ok');
...
document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') {
        if (document.getElementById('gw-velo').classList.contains('abierto')) {
            cerrarGatewayWISP();
            return;
        }
        if (document.getElementById('sshadmin-velo').classList.contains('abierto')) {
            cerrarSSHAdmin();
            return;
        }
        if (document.getElementById('m-firewall').classList.contains('abierto')) {
            cerrarFirewall();
            return;
        }
        if (document.getElementById('fx-velo').classList.contains('abierto')) {
            cerrarArchivos();
            return;
        }
        if (document.getElementById('tools-velo').classList.contains('abierto')) {
            cerrarHerramientas();
            return;
        }
        if (document.getElementById('term-velo').classList.contains('abierto')) {
            cerrarTerminalUI();
            return;
        }
        if (document.getElementById('m-huella').classList.contains('abierto')) {
            document.getElementById('huella-no').click();
            return;
        }
        if (document.getElementById('m-pass').classList.contains('abierto')) {
            document.getElementById('pass-no').click();
            return;
        }
        if (document.getElementById('m-seguridad').classList.contains('abierto')) {
            document.getElementById('seg-no').click();
            return;
        }
    }
    if (e.ctrlKey && e.key === 'Enter') {
        if (document.getElementById('m-pass').classList.contains('abierto')) {
            document.getElementById('pass-si').click();
            return;
        }
        var activo = document.activeElement;
        if (activo && activo.closest && activo.closest('#form')) {
            guardarServidorDesdeForm();
        }
    }
});
...
function mostrarVista(nombre) {
    ...
    var cs = document.querySelector('.content-shell');
    if (cs)
        cs.scrollTop = 0;
    if (nombre === 'profiles')
        abrirPlegable('form-cabecera', 'form');
    if (nombre === 'backup')
        abrirPlegable('copia-cabecera', 'copia-caja');
    if (nombre === 'updates')
        abrirPlegable('upd-cabecera', 'upd-caja');
    if (nombre === 'monitoring')
        cargarMonitoring();
    if (nombre === 'wireguard')
        cargarWireGuard();
...
function sshPortLog(text, tipo, reset) { var n = $('ssh-port-console'); if (!n)
    return; if (reset)
    vaciar(n); var l = document.createElement('div'); if (tipo)
    l.className = tipo; l.textContent = text; n.appendChild(l); n.scrollTop = n.scrollHeight; }
...
function abrirSSHAdmin(nombre) {
    sshAdminServidor = nombre;
    sshPortToken = null;
    var p = perfilCache(nombre) || {};
    $('sshadmin-srv').textContent = nombre;
    $('ssh-key-path').value = p.key || '';
    $('ssh-key-pass').value = '';
    $('ssh-key-recordar').checked = false;
    $('ssh-port-actual').textContent = p.puerto || 22;
    $('ssh-port-nuevo').value = (p.puerto || 22) === 2222 ? 22022 : 2222;
    $('ssh-port-sudo').value = '';
    $('ssh-port-aplicar').disabled = true;
    $('ssh-port-cancelar').disabled = true;
    $('ssh-port-probar').disabled = false;
    sshSteps('', '', '');
    sshStatus('ssh-key-status', p.key ? 'Key actual: ' + p.key : 'Este perfil no tiene una key asignada todavía.', '');
    sshStatus('ssh-port-status', 'Todavía no se ha cambiado nada.', '');
    sshPortLog('Esperando una prueba. La conexión actual no se cerrará.', '', true);
    cerrarHerramientas();
    $('sshadmin-velo').classList.add('abierto');
}
function cancelarPruebaPuerto(cerrarDespues) {
    ...
    return api('/api/herramientas/ssh-puerto/cancelar', { method: 'POST', body: JSON.stringify({ servidor: srv, token: tok, sudoPassword: $('ssh-port-sudo').value }) }).then(function (r) {
        if (r.error) {
            sshStatus('ssh-port-status', r.error, 'err');
            throw new Error(r.error);
        }
        sshPortToken = null;
        $('ssh-port-aplicar').disabled = true;
        ...
        $('ssh-port-probar').disabled = false;
        sshSteps('', '', '');
        sshStatus('ssh-port-status', 'Prueba cancelada. Se restauró el puerto anterior.', '');
        sshPortLog('↩ Prueba cancelada: configuración anterior restaurada.', 'warn');
        if (cerrarDespues)
            $('sshadmin-velo').classList.remove('abierto');
    });
}
function cerrarSSHAdmin() {
    if (sshPortToken) {
        if (!confirm('Hay una prueba de puerto pendiente. ¿Cancelar la prueba y restaurar el puerto anterior antes de cerrar?'))
            return;
...
$('tool-key').onclick = function () { if (!toolsServidor) {
    aviso('No hay servidor seleccionado.', false);
    return;
} abrirSSHAdmin(toolsServidor); };
$('tool-ssh-port-btn').onclick = function () { if (!toolsServidor) {
    aviso('No hay servidor seleccionado.', false);
    return;
} abrirSSHAdmin(toolsServidor); setTimeout(function () { $('ssh-port-nuevo').focus(); }, 60); };
...
$('ssh-key-usar').onclick = function () {
    if (!sshAdminServidor)
        return;
    var key = $('ssh-key-path').value.trim();
    if (!key) {
        sshStatus('ssh-key-status', 'Selecciona primero una key privada.', 'warn');
        return;
    }
    var b = this;
    b.disabled = true;
    b.textContent = 'Probando…';
    sshStatus('ssh-key-status', 'Abriendo una segunda conexión SSH con esta key…', '');
    api('/api/herramientas/usar-key', { method: 'POST', body: JSON.stringify({ servidor: sshAdminServidor, key: key, passphrase: $('ssh-key-pass').value, recordar: $('ssh-key-recordar').checked }) }).then(function (r) {
        if (r.necesitaPassphrase) {
            sshStatus('ssh-key-status', 'La key necesita passphrase. Escríbela arriba y vuelve a probar.', 'warn');
            return;
        }
        if (r.error) {
            sshStatus('ssh-key-status', r.error, 'err');
            return;
        }
        $('ssh-key-path').value = r.key || key;
        sshStatus('ssh-key-status', '✓ Key instalada/comprobada y asignada al perfil · ' + (r.tipo || '') + ' · ' + (r.huellaKey || '') + (r.publicaInstalada ? ' · pública añadida al servidor' : ' · pública ya estaba instalada'), 'ok');
        refrescarLista();
        aviso('Key SSH comprobada y asignada al perfil.', true);
...
$('ssh-key-crear').onclick = function () {
    if (!sshAdminServidor)
        return;
    if (!confirm('Se generará una ED25519 nueva en este equipo y se instalará únicamente su clave pública en ' + sshAdminServidor + '. ¿Continuar?'))
        return;
    var b = this;
    b.disabled = true;
    b.textContent = 'Creando…';
    sshStatus('ssh-key-status', 'Generando, instalando la pública y abriendo una segunda conexión de prueba…', '');
    api('/api/herramientas/crear-key', { method: 'POST', body: JSON.stringify({ servidor: sshAdminServidor }) }).then(function (r) {
        if (r.error) {
            sshStatus('ssh-key-status', r.error, 'err');
            return;
        }
        $('ssh-key-path').value = r.key || '';
        $('ssh-key-pass').value = '';
        sshStatus('ssh-key-status', '✓ ED25519 creada, instalada y comprobada. Key local: ' + (r.key || ''), 'ok');
        refrescarLista();
        aviso('Nueva key SSH instalada y comprobada.', true);
...
$('ssh-port-probar').onclick = function () {
    if (!sshAdminServidor)
        return;
    var puerto = parseInt($('ssh-port-nuevo').value, 10);
    if (!puerto || puerto < 1 || puerto > 65535) {
        sshStatus('ssh-port-status', 'Puerto inválido.', 'err');
        return;
    }
    var b = this;
    b.disabled = true;
    sshSteps('active', '', '');
    sshStatus('ssh-port-status', 'Preparando ambos puertos y ejecutando sshd -t…', '');
    sshPortLog('1/3  Manteniendo abierto el puerto actual y preparando ' + puerto + '…', '', true);
    sshPortLog('     Validando firewall y sshd_config…', '');
    api('/api/herramientas/ssh-puerto/probar', { method: 'POST', body: JSON.stringify({ servidor: sshAdminServidor, puerto: puerto, sudoPassword: $('ssh-port-sudo').value }) }).then(function (r) {
        if (r.error) {
            sshSteps('', '', '');
            sshStatus('ssh-port-status', r.error, 'err');
            sshPortLog('ERROR  ' + r.error, 'err');
            return;
        }
        sshPortToken = r.token;
        sshSteps('ok', 'ok', '');
        $('ssh-port-aplicar').disabled = false;
        $('ssh-port-cancelar').disabled = false;
        $('ssh-port-probar').disabled = true;
        sshPortLog('2/3  ✓ Segunda conexión SSH interna verificada por ' + puerto + '.', 'ok');
        if (r.rollbackGuard)
            sshPortLog('     Protección activa: rollback automático en 5 min si el flujo queda abandonado.', 'warn');
        sshPortLog('     El perfil TODAVÍA usa el puerto anterior. Pulsa Aplicar para confirmar.', '');
        sshStatus('ssh-port-status', '✓ ' + (r.mensaje || ('Conexión real verificada por ' + puerto + '. Pulsa Aplicar para guardar el cambio.')), 'ok');
    }).catch(function () { sshSteps('', '', ''); sshStatus('ssh-port-status', 'Error de comunicación durante la prueba.', 'err'); sshPortLog('ERROR  No se pudo completar la prueba.', 'err'); }).finally(function () { if (!sshPortToken)
        b.disabled = false; });
};
$('ssh-port-aplicar').onclick = function () {
    if (!sshPortToken || !sshAdminServidor)
        return;
    var b = this;
    b.disabled = true;
    sshSteps('ok', 'ok', 'active');
    sshStatus('ssh-port-status', 'Aplicando el puerto probado y haciendo una comprobación final…', '');
    sshPortLog('3/3  Aplicando ' + $('ssh-port-nuevo').value + ' y ejecutando la comprobación final…', '');
    api('/api/herramientas/ssh-puerto/aplicar', { method: 'POST', body: JSON.stringify({ servidor: sshAdminServidor, token: sshPortToken, cerrarAnterior: $('ssh-port-close-old').checked, sudoPassword: $('ssh-port-sudo').value }) }).then(function (r) {
        ...
        $('ssh-port-cancelar').disabled = true;
        $('ssh-port-probar').disabled = false;
        $('ssh-port-actual').textContent = r.puerto;
        sshStatus('ssh-port-status', '✓ ' + (r.mensaje || ('Puerto ' + r.puerto + ' aplicado y perfil actualizado.')), 'ok');
        sshPortLog('✓ CAMBIO CONFIRMADO. Perfil actualizado a ' + r.puerto + '.', 'ok');
        refrescarLista();
        aviso('Puerto SSH cambiado y verificado.', true);
...
function abrirGWTerminal(nombre) {
    if (typeof Terminal !== 'function' || typeof FitAddon === 'undefined' || typeof FitAddon.FitAddon !== 'function') {
        aviso('No se pudo cargar xterm.js.', false);
        return;
    }
    cerrarGWTerminal();
    var n = $('gw-term');
    vaciar(n);
    gwTerm = new Terminal({ cursorBlink: true, fontSize: 12, fontFamily: "'JetBrains Mono', Consolas, monospace", theme: { background: '#050c12', foreground: '#d8e4ea', cursor: '#4de19a', selectionBackground: '#1f3b49' } });
    gwFit = new FitAddon.FitAddon();
    ...
    gwWs.onopen = function () { if (gwTerm) {
        gwWs.send(JSON.stringify({ cols: gwTerm.cols, rows: gwTerm.rows }));
        gwTerm.focus();
        if (gwPendingCmd) {
            var cmd = gwPendingCmd;
            gwPendingCmd = null;
            setTimeout(function () { enviarGW(cmd); }, 80);
        }
    } };
    gwWs.onmessage = function (e) { if (gwTerm)
        gwTerm.write(new Uint8Array(e.data)); };
    gwWs.onerror = function () { if (gwTerm)
        gwTerm.write('\r\n\x1b[31m[error en terminal Gateway WISP]\x1b[0m\r\n'); };
    gwWs.onclose = function () { if (gwTerm)
        gwTerm.write('\r\n\x1b[33m[sesión terminada]\x1b[0m\r\n'); };
    gwTerm.onData(function (d) { if (gwWs && gwWs.readyState === 1)
        gwWs.send(enc.encode(d)); });
    gwTerm.onResize(function (t) { if (gwWs && gwWs.readyState === 1)
        gwWs.send(JSON.stringify({ cols: t.cols, rows: t.rows })); });
    setTimeout(function () { if (gwFit) {
        gwFit.fit();
        gwTerm.focus();
    } }, 80);
}
function cerrarGWTerminal() { if (gwWs) {
    gwWs.close();
    gwWs = null;
} if (gwTerm) {
    gwTerm.dispose();
    ...
    vaciar($('gw-term')); }
function enviarGW(cmd) { if (!gwWs || gwWs.readyState !== 1) {
    gwPendingCmd = cmd;
    if (gwTerm)
        gwTerm.write('\r\n\x1b[33m[esperando a que la terminal esté lista…]\x1b[0m\r\n');
    return;
} gwTerm.clear(); gwTerm.write('\x1b[2J\x1b[H'); gwWs.send(new TextEncoder().encode(cmd + '\r')); gwTerm.focus(); }
...
function prepararGWPaquete() { if (!gwServidor || gwPreparing)
    return Promise.resolve(false); gwPreparing = true; gwPackageReady = false; gwBotones(true); $('gw-package-state').textContent = 'Preparando paquete…'; $('gw-package-state').className = 'gw-chip'; return api('/api/herramientas/gateway-wisp/preparar', { method: 'POST', body: JSON.stringify({ servidor: gwServidor }) }).then(function (r) { if (r.error) {
    $('gw-package-state').textContent = 'Error de paquete';
    $('gw-package-state').className = 'gw-chip';
    if (gwTerm)
        gwTerm.write('\r\n\x1b[31m[No pude preparar el paquete: ' + r.error + ']\x1b[0m\r\n');
    return false;
} gwPackageReady = true; $('gw-bundle-ver').textContent = 'v' + (r.versionPaquete || '?'); $('gw-package-state').textContent = '✓ Paquete listo'; $('gw-package-state').className = 'gw-chip ok'; gwBotones(false); if (gwTerm)
...
function cargarGWEstado() {
    if (!gwServidor)
        return;
    $('gw-state-main').textContent = 'Consultando…';
    api('/api/herramientas/gateway-wisp/estado?servidor=' + encodeURIComponent(gwServidor)).then(function (r) {
        if (r.error) {
            $('gw-state-main').textContent = 'No disponible';
            return;
        }
        $('gw-bundle-ver').textContent = 'v' + (r.versionPaquete || '?');
        $('gw-installed-ver').textContent = (r.version && r.version !== 'absent') ? r.version : '—';
        $('gw-state-main').textContent = r.instalado ? '● Gateway detectado' : '○ No instalado';
        $('gw-state-main').className = 'gw-chip ' + (r.instalado ? 'ok' : '');
        document.querySelectorAll('#gw-components .gw-component').forEach(function (row) { var c = row.dataset.comp; var st = (r.componentes || {})[c]; var dot = row.querySelector('.gw-dot'); if (dot)
            dot.className = 'gw-dot ' + (st === 'installed' ? 'ok' : ''); });
    }).catch(function () { $('gw-state-main').textContent = 'Error consultando'; });
}
function abrirGatewayWISP(nombre) { gwServidor = nombre; gwPackageReady = false; gwPendingCmd = null; $('gw-srv').textContent = nombre; cerrarHerramientas(); $('gw-velo').classList.add('abierto'); abrirGWTerminal(nombre); cargarGWEstado(); prepararGWPaquete(); setTimeout(function () { if (gwFit)
    gwFit.fit(); }, 100); }
...
$('tool-gw-install').onclick = function () { if (!toolsServidor) {
    aviso('No hay servidor seleccionado.', false);
    return;
} abrirGatewayWISP(toolsServidor); };
function gwAccion(accion, comp) {
    if (!gwServidor)
        return;
    if ((accion === 'desinstalar-todo' || accion === 'desinstalar-componente') && !confirm(accion === 'desinstalar-todo' ? 'Se creará un backup y se retirará la integración Gateway WISP sin purgar paquetes de terceros. ¿Continuar?' : '¿Quitar de forma conservadora la integración de este componente?'))
        return;
    var ejecutar = function () { gwBotones(true); return api('/api/herramientas/gateway-wisp/comando', { method: 'POST', body: JSON.stringify({ servidor: gwServidor, accion: accion, componente: comp || '' }) }).then(function (r) { if (r.error) {
        aviso(r.error, false);
        return;
    } enviarGW(r.comando); }).catch(function () { aviso('No pude preparar el paquete Gateway WISP.', false); }).finally(function () { gwBotones(false); }); };
    if (!gwPackageReady) {
        prepararGWPaquete().then(function (ok) { if (ok)
            ejecutar(); });
    }
    else
        ejecutar();
}
$('gw-install-all').onclick = function () { gwAccion('instalar-todo', ''); };
$('gw-uninstall-all').onclick = function () { gwAccion('desinstalar-todo', ''); };
...
function cambiarPuertoFirewall(accion, boton) {
    if (!firewallServidor || !firewallEditable)
        return;
    var puerto = parseInt($('fw-puerto').value, 10);
    if (!puerto || puerto < 1 || puerto > 65535) {
        aviso('Escribe un puerto entre 1 y 65535.', false);
        return;
    }
...
document.getElementById('btn-probar-key').onclick = function () {
    ...
    if (!key) {
        res.textContent = 'Escribe o busca la ruta de la clave.';
        return;
    }
    ...
    api('/api/probar-key', { method: 'POST', body: JSON.stringify({
        ...
        res.textContent = 'Clave válida (' + r.tipo + ') · ' + r.huella;
...
function abrirArchivos(nombre) {
    ...
    if (nombre && Array.prototype.some.call(sel.options, function (o) { return o.value === nombre; }))
        sel.value = nombre;
    ...
    if (fxServidor)
        ...
        mensajeEn(document.getElementById('fx-lista-rem'), 'vacio-fx', 'Conecta un servidor para explorar sus archivos.');
}
function cerrarArchivos() {
    ...
    if (destinoAnterior === 'ssh-tool-key' && sshAdminServidor)
        document.getElementById('sshadmin-velo').classList.add('abierto');
}
...
function refrescarSelectorServidores() {
    ...
    if (activos.length) {
        ...
        var o = nodo('option', '', '— sin conexiones activas —');
        o.value = '';
        sel.appendChild(o);
    }
    if (activos.indexOf(previo) !== -1)
        sel.value = previo;
}
...
function cargarRemoto(ruta) {
    if (!fxServidor) {
        aviso('Elige un servidor conectado.', false);
        return;
    }
    api('/api/archivos', { method: 'POST', body: JSON.stringify({ servidor: fxServidor, ruta: ruta }) })
        ...
        d.archivos.forEach(function (a) {
            ...
            if (a.carpeta) {
                it.onclick = function (ev) { if (ev.target.tagName !== 'BUTTON')
                    cargarRemoto(a.ruta); };
            }
            else {
                ...
                bD.onclick = function (ev) {
                    ...
                    api('/api/archivos/descargar', { method: 'POST', body: JSON.stringify({
                        ...
                        if (r.error)
                            ...
                            aviso('Descargado: ' + r.destino, true);
                            cargarLocal(fxRutaLoc);
                        }
            ...
            bB.className = 'peligro';
            bB.textContent = '✕';
            bB.title = 'Borrar';
            bB.onclick = function (ev) {
                ev.stopPropagation();
                if (!confirm('¿Borrar ' + a.nombre + ' en el servidor?'))
                    return;
                api('/api/archivos', { method: 'POST', body: JSON.stringify({
        ...
        d.archivos.forEach(function (a) {
            ...
            if (a.carpeta) {
                ...
                if (selectorDestino === 'ssh-tool-key') {
                    var bKU = document.createElement('button');
                    bKU.className = 'principal';
                    bKU.textContent = 'Usar';
                ...
                if (selectorDestino === 'script') {
                    ...
                    bE.className = 'principal';
                    bE.textContent = 'Ejecutar';
                    ...
                    bS.textContent = '⬆ Subir';
                    bS.onclick = function (ev) {
                        ev.stopPropagation();
                        if (!fxServidor || !fxRutaRem) {
                            aviso('Abre primero una carpeta del servidor.', false);
                            return;
                        }
                        bS.disabled = true;
                        bS.textContent = '…';
...
document.getElementById('fx-abrir').onclick = function () {
    ...
    document.getElementById('fx-srv').textContent = fxServidor || 'solo equipo local';
    if (!fxServidor) {
        aviso('No hay conexiones activas.', false);
        return;
    }
    cargarRemoto('');
    if (!fxRutaLoc)
