...
// ── WireGuard local ───────────────────────────────────
var wgcProfiles = [], wgcSelected = null, wgcStatus = {}, wgcPrev = {}, wgcTimer = null, wgcSecretsVisible = false;
...
function wgcSetSnapshot(s) { s = s || {}; var connected = !!s.connected; $('wgc-state').textContent = connected ? 'Conectado' : 'Desconectado'; $('wgc-state').className = 'wgclient-state' + (connected ? ' up' : ''); $('wgc-connect').hidden = connected; $('wgc-disconnect').hidden = !connected; $('wgc-connect').disabled = !wgcSelected; $('wgc-interface').textContent = s.interface || '—'; $('wgc-rx-total').textContent = wgcBytes(s.rxBytes || 0) + ' total'; $('wgc-tx-total').textContent = wgcBytes(s.txBytes || 0) + ' total'; $('wgc-handshake').textContent = wgcHandshake(s.latestHandshake); var p = wgcSelected ? wgcProfileById(wgcSelected) : null; $('wgc-endpoint-summary').textContent = wgcEndpoint(p); var rate = wgcSelected && wgcStatus[wgcSelected] && wgcStatus[wgcSelected]._rate || {}; $('wgc-rx').textContent = wgcMbit(rate.rx || 0); $('wgc-tx').textContent = wgcMbit(rate.tx || 0); var warn = $('wgc-runtime-warning'); warn.hidden = !s.error; warn.textContent = s.error || ''; wgcUpdatePeerLive(s); }
...
} wgcSelected = r.profile.id; $('wgc-private').value = ''; wgcNote('Perfil guardado. PrivateKey y PresharedKey quedaron cifradas.', 'ok'); return api('/api/wireguard/perfiles'); }).then(function (list) { wgcProfiles = list || []; wgcLoadProfile(wgcSelected); if (connectAfter)
...
$('wgc-connect').onclick = function () { wgcSave(true).catch(function () { }); };
